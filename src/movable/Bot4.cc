...
Bot4::Bot4(Location l, Location mapSize, TileColor ac)
    : Actor(l, ac),
...
BotPlayingReturn Bot4::getMove(const Game &game, TileColor color1, TileColor color2, double timeLimit) {
...
bool Bot4::exceedTimeLimit(sf::Clock &clock) {
    return clock.getElapsedTime().asMilliseconds() >= timeLimit;
}

double Bot4::minimax(int depth, bool maximizingPlayer, sf::Clock &clock, int &position) {
